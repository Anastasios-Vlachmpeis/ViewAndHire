...
def analyze_audio_segment(wav_path: str, transcript_text: str = "") -> dict[str, Any]:
    with wave.open(wav_path, "rb") as audio:
        if audio.getnframes() == 0:
            return {"duration": 0.0, "score": None, "features": {}, "pauses": [],
    ...
    if not math.isfinite(mean_f0):
        return {"duration": float(duration), "score": None, "features": {}, "pauses": [],
    ...
    return {
        ...
        "features": features,
        "pauses": pauses,
        "notes": "Observable delivery only. These measurements do not assess confidence or competency.",
    }
